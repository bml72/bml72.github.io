...
// Create the variable for the SVG and reference the prop_deaths div when creating it.
var prop_svg_deaths = d3.select('#prop_deaths')
    .append('svg')
...
d3.json('../../data/cleaned/json/proportionate_total_deaths.json').then(function(mydata){
    mydata.forEach(function(d){
        d.proportionate_total_deaths = +d.proportionate_total_deaths;
    });
    ...
    var y = d3.scaleLinear()
        .domain([0, d3.max(mydata, function(d){ return d.proportionate_total_deaths; }) + 0.005])
        .range([height, 0]);
    ...
    prop_svg_deaths.append('text')
        ...
        .text('Proportionate Total Deaths (% of Pop)')
        .style('font', '15.5px times');
    ...
    prop_svg_deaths.append('text')
        ...
        .attr('font-size', '20px')
        .attr('text-anchor', 'middle')
        .text('Top Ten Countries by Proportionate Total Deaths')
        .style('font', '15.5px times');

    // Assigning the tooltip and specifying what to display when a bar is hovered over.
    var tip = d3.tip().attr('class', 'd3-tip')
        .html(function(d){
            var text = "Country: <span style='color: #3EA199'>" + d.country + '</span><br>';
                text += "Population: <span style='color: #FFF74A; text-transform: capitalize'>" + d3.format(',')(d.population) + '</span><br>';
                text += "Total Deaths (% of Pop): <span style='color: #FFF74A; text-transform: capitalize'>" + d3.format(',%')(d.proportionate_total_deaths) + '</span><br>';
            return text;
        });
    
    // Call the previously-defined tip variable to the SVG.
    prop_svg_deaths.call(tip);

    // Create the "bars" in the histogram. ".on" defines what to highlight the bars with when the mouse hovers over them.
    var rects = prop_svg_deaths.selectAll('rect')
        .data(mydata)
            .enter().append('rect')
                .attr('y', function(d, i){ return y(d.proportionate_total_deaths); })
                .attr('x', function(d, i){ return x(d.country); })
                .attr('width', x.bandwidth)
                .attr('height', function(d, i){ return height - y(d.proportionate_total_deaths); })
                .attr('fill', '#3EA199')
